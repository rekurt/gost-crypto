...
- [x] Перечитать README.md целиком — проверить что все примеры кода компилируются, import paths корректны, ссылки не битые, структура проекта соответствует реальности
- [x] Перечитать README.ru.md целиком — аналогичная проверка, плюс убедиться что русская версия полностью синхронизирована с английской
- [x] Перечитать API.md — проверить что все описанные функции, типы, константы существуют в коде; удалить описание несуществующих, добавить описание отсутствующих
- [x] Перечитать API.ru.md — аналогичная проверка + синхронизация с API.md
- [x] Проверить SECURITY.md и CONTRIBUTING.md — убедиться что ссылки, имена, контактные данные корректны
- [x] Проверить все `// Package ...` doc-комментарии — запустить `go doc ./...` и убедиться что описания корректны и полны
- [x] Проверить все `Example*` функции — запустить `go test -run Example ./...` и убедиться что Output-комментарии совпадают с реальным выводом
- [x] Проверить что все примеры в `_examples/` компилируются: `go build ./_examples/...`
- [x] Сверить CLAUDE.md с текущим состоянием кода — обновить сигнатуры функций, покрытие, статус реализации
- [x] Финальная проверка: нет ли расхождений между документацией и кодом (grep по именам функций в .md файлах и сверка с реальными сигнатурами)
