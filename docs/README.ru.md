...
- **Без Go-зависимостей** — в `go.mod` нет директив `require`; только CryptoPro CSP + CGO при сборке
...
- Go 1.22 или новее
- CryptoPro CSP 5.0+ для Linux ([инструкция по установке](DEPLOYMENT.md))
- CGO включён
...
1. Каждая подпись использует криптографически случайный нонс (k) через CryptoPro CSP
...
4. Криптографические операции делегируются CryptoPro CSP
