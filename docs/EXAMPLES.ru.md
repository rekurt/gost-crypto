...
func main() {
    key := make([]byte, 32) // ключ Кузнечик — 32 байта
    rand.Read(key)
...
`pkg/hd` детерминированно выводит **и** цепные коды, **и** приватные ключи
из сида через HKDF-Стрибог. Один и тот же сид и путь всегда дают один и тот
же ключ ГОСТ Р 34.10-2012. Если байты из HKDF выпадают за пределы [1, q-1],
автоматически применяется rejection sampling с детерминированным пересчётом.
